...
namespace Ryne.ReportingSystem.Application
{
    ...
    public class CreateDocuments
    {
        ...
        public async Task CreateActOfRepairWork()
        {
            //FileUtil.OutputDir = new DirectoryInfo($"{AppDomain.CurrentDomain.BaseDirectory}SampleApp");
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;


            var exelfile = new FileInfo(@"F:\project\Ryne.ReportingSystem\Ryne.ReportingSystem.ConsoleClient\акты рем. работ 2022.xlsx");
            Console.WriteLine(exelfile);
            using var package = new ExcelPackage(exelfile);
            await package.LoadAsync(exelfile);
            ExcelWorksheet worksheet = package.Workbook.Worksheets[1];
            worksheet.Cells["E3"].Value = 1000;
